...
def q_learning(
    ...
    while not done:
        ...
        if "winner" in played_game_status:
            ...
            pop_state_for_o = list_of_available_states_copy.pop()
            played_game_status = new_game.send(
                {
                    'row': pop_state_for_o['row'],
                    'column': pop_state_for_o['column']
                }
            )

            list_of_played_positions.append(
                (pop_state_for_o['row'], pop_state_for_o['column']))

            if "winner" in played_game_status:
                if played_game_status["winner"] == "O":
                    reward = -10
                # column_position_on_q_table -= 1
...
if __name__ == '__main__':
    ...
    epsilon = 0.2
    q_learning(
        q_table_to_update=q_table_to_update_new,
